...
function Aeronaves() {
  ...
  async function handleRegistrarTestes() {
    if (!selecionada) return;
    try {
      const res = await api.post(`/aeronaves/${selecionada.id}/testes`, {
        aerodinamico: testeAer,
        hidraulico: testeHid,
        eletrico: testeEle,
      });
      setSelecionada((prev) => (prev ? { ...prev, testes: [res.data] } : prev));
      setAeronaves((prev) => prev.map((a) => (a.id === selecionada.id ? { ...a, testes: [res.data] } : a)));
      modalTestes.fechar();
            ...
            {selecionada ? (
              <div className="flex flex-col h-full bg-white rounded-2xl border border-slate-200 p-6 md:p-8 shadow-sm">
                          ...
                          <div key={teste.id} className="flex flex-col gap-1">
                            {[
                              ...
                              <div key={t.label} className="flex items-start gap-2">
                                <div
                                  className={`mt-1.5 w-1.5 h-1.5 rounded-full shrink-0 ${t.valor === "REPROVADO" ? "bg-red-400" : "bg-emerald-400"}`}
                                />
                ...
                <button
                  onClick={async () => {
                    try {
                      const res = await api.get(`/aeronaves/${selecionada.id}/relatorio`, {
                        responseType: "blob",
                      });
                  ...
                  className="flex-1 flex items-center justify-center gap-2 py-2.5 mt-6 bg-[var(--azul-escuro)] border border-slate-300 text-white rounded-lg hover:bg-[var(--azul)] hover:border-slate-400 transition-all text-sm font-semibold cursor-pointer shadow-sm">
                  Gerar Relatório
                </button>
